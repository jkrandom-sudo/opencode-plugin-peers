...
- **Discovery**: protocol v2 publishes one `0600` registry record per session endpoint and one v1 compatibility record for the most recently active root session. Readers accept both versions. Duplicate names are displayed with short endpoint IDs and can only be targeted by name when unique.
